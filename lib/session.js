    ...
    INSERT DATA {
      GRAPH ${sparqlEscapeUri(SESSIONS_GRAPH)} {
        ${sparqlEscapeUri(accountUri)}
          ...
          foaf:firstName ${sparqlEscapeString(userInfo.given_name || '')} ;
          foaf:lastName ${sparqlEscapeString(userInfo.family_name || '')}
          ${rrnTriple} .
      }
    }
  `);
...
export async function getSessionInfo(sessionUri) {
  const result = await query(`
    PREFIX foaf: <http://xmlns.com/foaf/0.1/>
    PREFIX ext:  <http://mu.semte.ch/vocabularies/ext/>
    ...
    SELECT ?firstName ?lastName WHERE {
      GRAPH ${sparqlEscapeUri(SESSIONS_GRAPH)} {
        ...
        OPTIONAL { ?account foaf:lastName ?lastName }
      }
    } LIMIT 1
  `);
  ...
  return {
    firstName: binding.firstName?.value || null,
    lastName: binding.lastName?.value || null,
  };
}
